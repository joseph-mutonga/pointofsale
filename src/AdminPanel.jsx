...
export default function AdminPanel({ user, onBack, onGoToPOS }) {
    ...
    const handleSaveItem = async (e) => {
        ...
        if (res.success) { setItemEdit(null); loadAll(); }
    ...
    const handleSaveUser = async (e) => {
        ...
        if (res.success) { setUserEdit(null); loadAll(); }
        else alert(res.message);
    };

    const handleToggleUserStatus = async (user) => {
        const newStatus = user.status === 'active' ? 'disabled' : 'active';
        if (!confirm(`Are you sure you want to ${newStatus === 'active' ? 'activate' : 'deactivate'} user ${user.username}?`)) return;
    ...
    const handleAddMaterial = async (e) => {
        ...
        if (res.success) { e.target.reset(); loadAll(); }
        else alert(res.message);
    };
            ...
            <header className="header" style={{ background: '#1e293b', color: 'white', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <button className="btn" style={{ background: '#059669', color: 'white' }} onClick={async () => {
                        ...
                        if (res.success) alert(`Success! The backup on your drive was REPLACED with current data.\nPath: ${res.path}`);
                        else if (res.message !== 'Backup cancelled') alert(`Backup failed: ${res.message}`);
                                            ...
                                            <tr key={s.id}>
                                                ...
                                                <td>
                                                    <span style={{
                                                        ...
                                                        padding: '2px 6px',
                                                        borderRadius: '4px',
                                                        background: s.type === 'service' ? '#f0fdf4' : s.type === 'layaway' ? '#eff6ff' : '#f8fafc',
                                                        color: s.type === 'service' ? '#166534' : s.type === 'layaway' ? '#1e40af' : '#64748b',
                                                        border: `1px solid ${s.type === 'service' ? '#bbf7d0' : s.type === 'layaway' ? '#bfdbfe' : '#e2e8f0'}`,
                                                        fontWeight: 'bold'
                                                    }}>
                                                        {(s.type || 'pos').toUpperCase()}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span style={{
                        ...
                        <div>
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                                {['daily', 'weekly', 'monthly', 'yearly'].map(r => (
                                    <button key={r} className="btn" style={{ background: reportRange === r ? '#3b82f6' : '#eee' }} onClick={() => setReportRange(r)}>{r}</button>
                                ))}
                            </div>
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        <form onSubmit={async (e) => {
                                            ...
                                            if (!workerId) { alert('Please select a staff member'); return; }
                                                ...
                                                <tbody>
                                                    {productionLogs.filter(l => !workforceDateFilter || new Date(l.created_at).toLocaleDateString('en-CA') === workforceDateFilter).map(l => (
                                                        <tr key={l.id}>
                                                            ...
                                                            <td>
                                                                <button className="btn btn-danger" style={{ padding: '2px 6px', fontSize: '0.7rem' }} onClick={async () => {
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        <form onSubmit={async (e) => {
                                            ...
                                            const res = await window.api.addWorkforcePayment({
                                                worker_id: workerId,
                                                worker_name: workerName,
                                                amount: Number(fd.get('amount')),
                                                payment_mode: fd.get('payment_mode'),
                                        ...
                                        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                                            <table style={{ fontSize: '0.9rem' }}>
                                                ...
                                                <tbody>
                                                    {workforcePayments.filter(p => !workforceDateFilter || new Date(p.date || p.created_at).toLocaleDateString('en-CA') === workforceDateFilter).map(p => (
                                                        <tr key={p.id}>
                        ...
                        <div style={{ position: 'relative', zIndex: 1 }}>
                            ...
                            <div style={{ marginBottom: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', fontSize: '0.9rem' }}>
                                <div>
                                    ...
                                    <div>{new Date(selectedSale.date).toLocaleString()}</div>
                                </div>
                                <div>
